...
public class Ex34 {
	public static void main(String[] args) {
		...
		*/
		...
		//사용자가 직접 밖에서 Calendar를 만들게 하지말고 클래스 안 메소드에서 Calendar를 만들어 해결하게 하면 좀 더 편리해진다. > 상황에 맞게 맞춰서 쓰기
...
//		hong.setBirthday(birthday);
		...
		hong.setFather(father); //자식에서 아빠 지정
		hong.setMother(mother); //자식에서 엄마 지정
		hong.hello();
		hong.info();
...
//	}
	...
	private boolean checkName(String name) {
		...
		if(name.length() < 2 || name.length() > 5) { //2자~5자가 아닐 때
			return false;
		}
		
		for(int i=0; i < name.length(); i++) {//한글인지 
	...
	// 자기소개하는 메소드
	public void info() {
		System.out.printf("안녕하세요. 저는 %s구요, %d살입니다.\n생일은 %tF입니다.\n아빠는 %s이고, 생일은 %s입니다.\n엄마는 %s이고, 생일은 %s입니다.\n"
