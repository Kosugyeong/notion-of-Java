...
public class Work {
	...
	private Scanner scan; // scanner를 모든 메소드에서 쓸 수 있도록 멤버 변수로 선언
	...
	private String getSeq() {
		...
		return (max + 1) + ""; // 뒤에 빈문자를 더하면 문자열로 만들 수 있다.!!!
	}
	...
	public void search() {
		...
		for (Student s : Data.slist) {
			...
			if (s.getName().contains(input)||s.getAddress().replace(" ", "").contains(input.replace(" ", ""))) { //부분검색 
