...
public class Ex62_File {
	...
	private static int count = 0; // 누적 변수
	...
	private static void m16() {
		...
		// HashSet(Collection) > Set(Interface)
		Set<String> set = map.keySet(); // Set을 구현한게 HashSet.. HashMap의 key들로만 구성된 Set
		...
		// 향상된 for문 구조(향상된 for문 내부가 iterator로 되어있다.)
		while (iter.hasNext()) {
	...
	private static void m15() {
		...
		// 파일 11,841, 폴더 2,504, 크기 794,264,238 바이트
	...
	private static void countFile(File dir) {
		...
		for (File subfile : list) {
			if (subfile.isFile()) {
				// count++;
				count += subfile.length(); // > (크기)바이트 구해줌
			}
		}
		...
		for (File subdir : list) {
			if (subdir.isDirectory()) {
				// count++;
				// 1~3번 반복
				countFile(subdir); // 재귀 호출 > 자식이 없는 폴더를 만날때까지 반복하게됨
			}
		}
	}
	...
	private static void m13() {
		...
		if (dir.exists()) {
			...
			// for(String f : list) {
			//
			// //이름 사용 > File 객체 생성 > ifFile(), isDirectory()
	...
	private static void m9() {
		...
		// System.out.println(c.getActualMaximum(Calendar.DAY_OF_YEAR)); //윤년이면 366, 평년이면 365
	...
	private static void m7() {
		...
		File dir = new File("C:\\class\\java\\file\\aaa\\bbb\\ccc");
	...
	private static void m5() {
		...
		File file = new File("C:\\class\\java\\file\\hello.txt"); // 참조 객체부터 만들기!!
		File file2 = new File("C:\\class\\java\\move\\hello.txt"); // 참조 객체부터 만들기!!
	...
	private static void m4() {
		...
		// - hello.txt -> hi.txt
		File file = new File("C:\\class\\java\\file\\hello.txt"); // 참조 객체부터 만들기!!
		File file2 = new File("C:\\class\\java\\file\\hi.txt"); // 참조 객체부터 만들기!!
	...
	private static void m2() {
		...
		if (dir.exists()) {
			...
			System.out.println(dir.length()); // 0 **폴더 크기 (파일이 폴더를 참조하고 있는것이므로 폴더 자체에는 내용이 하나도 없는것)
			System.out.println(dir.getAbsolutePath()); // C:\class\java\file\
			System.out.println(dir.getPath()); // C:\class\java\file\
			System.out.println(dir.lastModified()); // 1649031847284 > tick값
	...
	private static void m1() {
		...
		// 파일 참조 객체 > java.io.File (io는 inputout의 약자임)
		...
		if (file.exists()) {
			...
			System.out.println(file.lastModified()); // 1649031847284 > tick값
			System.out.println(file.isHidden()); // false


			// tick -> 년원일시분초로 바꾸고 싶으면
