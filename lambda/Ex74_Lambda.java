...
public class Ex74_Lambda {
	...
	public static void main(String[] args) {
		...
		/*
			...
			3. Function > 매개변수 O , 반환값 O
				...
				- Function<T,R> R은 return 값이다(반환값)
				- BiFunction<T,U,R>
				- andThen()
				- compose()
	...
	private static void m5() {
		...
		//- testXXX() 추상 메소드 제공
		//- 매개 변수를 전달받아 논리 검사 진행 > Boolean 반환
...
//함수형 인터페이스 (목적: 람다식 저장하려고 만든 인터페이스)
// > 함수형 인터페이스는 반드시 추상 메소드를 딱 1개만 가질 수 있다. > 이 1개의 추상 메소드가 람다식(익명 매스드)이 되기 때문에..
...
//람다식을 저장해야할 참조 변수가 필요해서 만든 인터페이스. 즉, 람다식 사용하려면 이 인터페이스가 반드시 필요하다. 
interface Test{
	int sum(int a, int b);
