...
public class Ex22_for {
	...
	private static void m13() {
		...
		int lastDay = getLastDay(year, month); // 월의 마지막일
		int dayOfWeek = getDayOfWeek(year, month); // year month가 무슨 요일인지!
		...
		for (int i = 1; i <= lastDay; i++) {
			...
			if ((i + dayOfWeek) % 7 == 0) { // 토요일이면 그 다음은 개행(모든달에 적용이되는 식)
				System.out.println();
			}
		}
	...
	private static void m12() {
		...
		System.out.printf("%d년 %d월 %d일은 %,d째 되는 날이고 %s요일입니다.", year, month, date, total,
				getDay(total));

	}

	private static int getLastDay(int year, int month) { // 윤년 계산으로 인해 year가 매개변수에 있어야함
