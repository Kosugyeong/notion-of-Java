...
public class Ex27_array {
	...
	private static void m7() {
		...
		//데이터 입력 > 문제의 요구사항에 따라 수정 > 문제 풀이 > i와 j를 어떻게 접근하느냐에 따라. 
		int n=1;
		for(int i=0;i<5;i++) { //이와 같이 문제 풀 때 일단 i<5와 같이 상수 쓰기 length쓰지 말고
			for(int j=0;j<5;j++) {
				nums[i][j]=n;
				n++;
			}
		...
		//데이터 출력 > 절대 수정 금지!!!!!!! > 항상 이렇게 그대로 써야함
		for(int i=0;i<5;i++) { 
			for(int j=0;j<5;j++) {
	...
	private static void m6() {
		...
		//변수의 초기값 > 실패하거나 아무일도 안일어났을 때 가져야될 값으로 초기화한다. 피해가 가장 적을것 같다고 생각하는 값.
		...
		for(int i=0;i<member.length;i++) {
			if(member[i].equals(search)) {
				result = true; //찾았다!!
				break; //불필요한 회전 중단하기 위해 찾으면 break
			}
		}
		...
		int index = -1; //발견 위치 > 배열의 인덱스로 가지지 않는 값으로 초기화를 해준다. 
